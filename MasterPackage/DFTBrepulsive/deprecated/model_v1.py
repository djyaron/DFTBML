...
# TODO: Optimize function call in spline generation (less important)
# TODO: Try exponential + spline?
...
def get_xydata_Z(model_Z, coefs, nvals=500, ider=0, xgrid=None):
...
def get_targets_from_dataset(target_type=None, fold=None, dataset_path=None,
                             data=None, in_memory=False):
    targets = dict()

    if in_memory:
        for mol, moldata in data.items():
            targets[mol] = moldata['target'] - moldata['baseline']
        return targets
        ...
        with File(dataset_path, 'r') as dataset:
            for mol, conf_arr in fold.items():
                ...
                if len(moldata) == 1:
                    ...
                    targets[mol] = moldata[0] - moldata[1] + moldata[2]
                else:
                    raise NotImplementedError
    else:
...
def get_predictions_from_dense(coefs_dense, fold=None, gammas_path=None,
                               gammas=None, in_memory=False):
    preds = dict()
    if in_memory:
        for mol, gamma_data in gammas.items():
            if mol == '_INFO':
                continue
            gamma = gamma_data['gammas']
...
def get_predictions_from_sparse(coefs_sparse, map_models, fold=None, gammas_path=None,
                                gammas=None, in_memory=False):
    coefs_dense = dense_coefs_from_sparse(map_models, coefs_sparse)
    if in_memory:
        preds = get_predictions_from_dense(coefs_dense, gammas=gammas, in_memory=in_memory)
        return preds
...
# TODO: rewrite this function
def create_dense(target_type=None, fold=None, dataset_path=None, gammas_path=None,
                 data=None, gammas=None, in_memory=False):
    if in_memory:
        targets = get_targets_from_dataset(data=data, in_memory=in_memory)
    else:
        # if target_type is a list of length 2, the target is the difference
        #   target = target_type[0] - target_type[1]
        dtypes = get_data_type(target_type)
        if len(dtypes) not in [1, 2, 3]:
            raise ValueError('create_dense: target_type is not of length 1 or 2 or 3')
    ...
    if in_memory:
        nconfs = sum([len(g['gammas']) for mol, g in gammas.items() if mol != '_INFO'])
    else:
        nconfs = fold.nconfs()
