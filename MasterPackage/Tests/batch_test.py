...
from DFTBLayer import DFTBList, create_batch, create_dataset, maxabs, np_segment_sum
...
@pytest.fixture
def test_dataset():
    def _test_dataset(data, smearing=None, print_all=True):
        ...
        for oname in data["onames"]:
            ...
            if oname != "R":
                for bsize in data["basis_sizes"]:
                    ...
                    calc[oname][bsize] = rot_out[gather].reshape(
                        [len(data["glabels"][bsize]), bsize, bsize]
                    )
        ...
        for bsize in data["basis_sizes"]:
            ...
            for igeom in range(ngeom):
                ep = np.dot(calc["G"][bsize][igeom, :, :], calc["dQ"][bsize][igeom, :]).reshape(
                    [1, bsize]
                )
                couMat[igeom, :, :] = -0.5 * calc["S"][bsize][igeom, :, :] * (ep + ep.T)
            calc["F"][bsize] = calc["H"][bsize] + couMat
            vals = net_vals[data["gather_for_rep"][bsize]]
            calc["Erep"][bsize] = np_segment_sum(vals, data["segsum_for_rep"][bsize])
        ...
        for bsize in data["basis_sizes"]:
            ...
            for igeom in range(ngeom):
                ...
                if smearing is None:
                    ...
                    fermi_level = determine_fermi_level(
                        Eorb,
                        2 * num_filled,
                        sigma=smearing["width"],
                        scheme=smearing["scheme"],
                        threshold=1.0e-10,
                    )
                    fermi_val = 2.0 / (1.0 + np.exp((Eorb - fermi_level) / sigma))
        ...
        for data_type, calc_by_bsize in calc.items():
            for bsize, calc_test in calc_by_bsize.items():
                err = np.max(np.abs((calc_test - data[data_type][bsize]).flatten()))
                if print_all:
...
def check_fock_routine(dftb):
    ...
    print("test of S diagonalization ", maxabs(np.dot(Svecs.T, np.dot(S, Svecs)) - np.diag(Svals)))
    phiS = np.dot(Svecs, np.diag(np.sqrt(1.0 / Svals)))
    print(
        "test of phiS.T S phiS = unit matrix ",
    ...
    if dftb.smearing is None:
        ...
        fermi_level = determine_fermi_level(
            w, 2 * nfilled, scheme=dftb.smearing_scheme, sigma=dftb.smearing, threshold=1.0e-10
        )
        occs = []
        for ei in w:
            occs.append(np.sqrt(0.5 * fermi(ei, fermi_level, dftb.smearing)))
...
def run_batch_tests(test_dataset):
    ...
    for mol_type in ["HAu2", "organic", "HCN"]:
        if mol_type == "organic":
            ...
            charges = [0] * 3 * ngeom
            gtypes = []
            geoms1 = random_triatomics(
                ngeom,
                [1, 6, 7],
                [0.7, 1.1],
                [0.7, 1.1],
                [(104.7 + 20.0) * math.pi / 180.0, (104.0 - 20.0) * math.pi / 180.0],
            )
            ...
            angle_range = [(104.7 + 20.0) * math.pi / 180.0, (104.0 - 20.0) * math.pi / 180.0]
            geoms1 = random_triatomics(ngeom, Zs, length_range1, length_range2, angle_range)
    ...
    if sigma is not None:
        smearing = {"scheme": "fermi", "width": sigma}
