...
def compare_feeds(reference_file: str, reconstituted_feeds: List[Dict]) -> None:
    ...
    for i in range(len(reference_file_feeds)):
        ...
        assert (
            set(curr_ref_fd["gather_for_rot"].keys()).difference(
                set(feedi["gather_for_rot"].keys())
            )
            == set()
        )
        assert (
            set(feedi["gather_for_rot"].keys()).difference(
                set(curr_ref_fd["gather_for_rot"].keys())
            )
            == set()
        )
        ...
        for shp in curr_ref_fd["rot_tensors"]:
            assert (
                (feedi["rot_tensors"][shp] is None) and (curr_ref_fd["rot_tensors"][shp] is None)
            ) or np.allclose(curr_ref_fd["rot_tensors"][shp], feedi["rot_tensors"][shp])
        ...
        for bsize in curr_ref_fd["glabels"].keys():
            ...
            for i in range(len(curr_ref_fd["dipole_mat"][bsize])):
                assert np.allclose(
                    curr_ref_fd["dipole_mat"][bsize][i], feedi["dipole_mat"][bsize][i]
                )
            ...
            for oper in curr_ref_fd["gather_for_oper"].keys():
                assert np.allclose(
                    curr_ref_fd["gather_for_oper"][oper][bsize],
                    feedi["gather_for_oper"][oper][bsize],
                )
...
def run_h5handler_tests():
    batch_filename = os.path.join(test_data_dir, "Fold0", "batches.h5")
    molec_filename = os.path.join(test_data_dir, "Fold0", "molecs.h5")
    reference_filename = os.path.join(test_data_dir, "Fold0", "reference_data.p")
    reconstituted_feeds = total_feed_combinator.create_all_feeds(
        batch_filename, molec_filename, True
    )
...
def run_safety_check(top_level_fold_path: str, fold_nums: List[int]) -> None:
    ...
    for num in fold_nums:
        ...
        reference_filename = os.path.join(
            os.getcwd(), top_level_fold_path, f"Fold{num}", "reference_data.p"
        )
        feeds = total_feed_combinator.create_all_feeds(
            batch_filename, molec_info_name, True
        )  # Dipoles always ragged, organized by bsize not num atom
