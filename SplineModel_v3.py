...
# def apply_linear_model(pars, c):
#     return np.dot(pars[0], c) + pars[1]
...
class LinearLoss:
    ...
    def value(self, coefs):
        res = np.dot(coefs.T, np.dot(self.alpha, coefs)) \
              - 2.0 * np.dot(self.beta.T, coefs) + self.const
        return res
...
def get_xydata_Z(model_Z, coefs, nvals=500):
    ...
    for Z, (rlow, rhigh) in cutoffs.items():
        ...
        y = models[Z].vals(coefs[loc[Z]], x)
        xydata[Z] = (x, y)
    return xydata


def plot_xydata_Z(xydata, syms=('b-', 'r-', 'g-', 'k-')):
    ...
    for Z in xydata[0].keys():
        ...
        for idata, xyZ in enumerate(xydata):
            (x, y) = xyZ[Z]
            plt.plot(x, y, syms[idata % len(syms)])
        plt.title(Z)
        iplot += 1
...
def get_predictions_from_dense(coefs_dense, map_models, fold, gammas_path):
...
def get_predictions_from_sparse(coefs_sparse, map_models, fold, gammas_path):
    ...
    preds = get_predictions_from_dense(coefs_dense, map_models, fold, gammas_path)
    return preds
...
def create_dense(target_type, fold, dataset_path, gammas_path):
    ...
    ntotal = 0
    with File(gammas_path, 'r') as gammas:
        # Make sure to use the same dataset that generated gammas
        # DO NOT checksum when create_dense() is in a loop
        # assert get_sha256(dataset_path) == gammas['_INFO']['SHA256'][()], \
        #     "create_dense() must use the same dataset that generated gammas"
        ...
        for mol, conf_arr in fold.items():
            ...
            alpha += np.dot(gamma.T, gamma)
            beta += np.dot(target, gamma)
            loss0 += np.sum(np.square(target))
    alpha /= ntotal * 1.0
    beta /= ntotal * 1.0
    loss0 /= ntotal * 1.0
