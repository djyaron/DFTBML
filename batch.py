...
from fermilevel import determine_fermi_level,fermi
...
def plot_models(data, figure_num = 300):
    try:
        plt.ion()
        for oper in data['onames'] + ['R']:
            figure_num += 1
            toplot = []
            for mod, raw in data['mod_raw'].items():
                if len(mod.Zs) == 2 and mod.oper == oper:
                        toplot.append(
                    {'oper' : mod.oper,
                     'name' : mod.oper+str(mod.Zs[0])+'-'+str(mod.Zs[1]) + '-'+mod.orb,
                       'r': [x.rdist for x in raw],
                       'dftb'  : [x.dftb  for x in raw]} )
...
#%%
#inv_map = {}
#for i,ele in iteritems(dftb_data['mod_raw']):
#    inv_map[i] = ele
"""
net_raw = []
for m in dftb_data['models']:
    net_raw.extend(dftb_data['mod_raw'][m])
#%%
for s,gather in dftb_data['gather_for_rot'].items():
    # if there are no rot_tensors for this shape, no rotation is needed
    print('shape ', s)
    if dftb_data['rot_tensors'][s] is None:
        for i0 in gather:
            print(i0, net_raw[i0])
    else:
        print('rot tensor shape ', dftb_data['rot_tensors'][s].shape)
        iarray = gather.reshape([-1,s[1]])
        for i0 in range(iarray.shape[0]):
            for i1 in range(iarray.shape[1]):
                print(i0,i1,net_raw[iarray[i0,i1]])

  

#%%
for k,v in dftb_data['gather_for_rot'].items():
    print('rot shape = ', k)
    for iele in v:
        print('   ', dftb_data['mod_raw'][iele])
