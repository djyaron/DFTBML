...
def create_graph_feed(config: Dict, batch: List[Dict], allowed_Zs: List[int]) -> (Dict, DFTBList):
    ...
    batch = create_batch(geom_batch, opers_to_model = config['opers_to_model'], FIXED_ZS=allowed_Zs) #Add in opers to model
...
def graph_generation(molecules: List[Dict], config: Dict, allowed_Zs: List[int], 
                     num_per_batch: int = 10) -> (List[Dict], List[DFTBList], List[List[Dict]]):
